...
void* threadFunc(void* argspt)
{
  ...
  int index = *(int*)argspt;
  
  int s = floor(width / (double)partition);
  int start = s * index;
  int end = s * (index + 1);
  if (index == partition - 1) end = width;
  ...
  for( int px = start; px<end; ++px )
    {
	  ...
	  for( int i=0; i<3; i++)
	    pixels[px][py][i] = max( min(scaled_color[i], 255), 0);
...
main( int argc, char **argv )
{
  ...
  partition = nthreads;
  int *indexes = malloc(sizeof(int) * nthreads);
  for (i = 0; i < nthreads; i++)
    indexes[i] = i;
  for (i = 0; i < nthreads; i++) {
    if ((ret = pthread_create(&threads[i], NULL, &threadFunc, (void *)&indexes[i])))
      {
